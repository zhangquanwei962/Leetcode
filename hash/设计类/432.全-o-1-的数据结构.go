...
func (l *AllOne) Inc(key string) {
	if cur := l.nodes[key]; cur != nil {
		curNode := cur.Value.(node)                                                    // 断言成node
		if nxt := cur.Next(); nxt == nil || nxt.Value.(node).count > curNode.count+1 { // nxt为0或者大于当前count+1
			l.nodes[key] = l.InsertAfter(node{map[string]struct{}{key: {}}, curNode.count + 1}, cur) // 更新nodes位置
		} else {
